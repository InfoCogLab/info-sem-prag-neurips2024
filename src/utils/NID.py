...
def get_NID(model, dataset, batch_size, field, glove_data=None, vae=None):
    ...
    proto_synonyms = find_synonyms(prototypes, 0.01) 
    ...
    for targ_idx in list(dataset.index.values):
        ...
        if speaker_obs != None: # i.e. we have the glove embeds    
            ...
            with torch.no_grad(): 
                ...
                #model_names.append(index_of_one)
    ...
    word_count_top = len(set(model_names))
   
    counter_humans = Counter(human_names)
    counter_model = Counter(model_names)
    
    total_humans = sum(counter_humans.values())
    total_model = sum(counter_model.values())
    
    prob_humans = np.array([v / total_humans for k, v in counter_humans.items()])
    prob_model = np.array([v / total_model for k, v in counter_model.items()])
    
    humans_above_k = sum(1 for key, value in counter_humans.items() if value > 1)
    model_above_k = sum(1 for key, value in counter_model.items() if value > 1)
    
    return NID(human_names, model_names), word_count_top, len(set(human_names)), model_above_k, humans_above_k, H(prob_model), H(prob_humans) 
