"""
Run model evaluation in pragmatics setting
"""
...
def run():
    ...
    for seed in settings.seeds:
        ...
        evaluation_data = pd.concat([test_data, val_data]).reset_index(drop=True)
            ...
            for folder_utility in [i for i in os.listdir(json_file_path) if "utility" in i]:
                ...
                try:
                    ...
                    for convergence_epoch in convergence_epochs:
                        ...
                        # we evaluate pragmatics on all the unseen data
                        eval_model_pragmatics(model, vae_model, c_dim, evaluation_data, viz_data, glove_data, num_cand_to_metrics, save_eval_path, fieldname='topname', calculate_complexity=do_calc_complexity, plot_comms_flag=do_plot_comms)
...
if __name__ == '__main__':
    ...
    #settings.kl_weights = [0.0001] # to evaluate the ranomly initialized model
