...
def get_feature_data(features_filename,
    ...
    """
    Load and process feature data from a file.

    Args:
        features_filename (str): The filename of the feature data file.
        desired_names (list, optional): A list of desired names to include in the dataset. Defaults to an empty list.
        excluded_names (list, optional): A list of names to exclude from the dataset. Defaults to an empty list.
        excluded_ids (list, optional): A list of IDs to exclude from the dataset. Defaults to an empty list.
        selected_fraction (float, optional): The fraction of names to randomly select from the dataset. Defaults to None.
        max_per_class (int, optional): The maximum number of samples to include per class. Defaults to None.

    Returns:
        pandas.DataFrame: The processed feature data.

    Raises:
        AssertionError: If both desired_names and excluded_names are specified.
...
if __name__ == "__main__":
    ...
    if see_distractors_pragmatics: 
        ...
        url_map = download_img()  # comment if no need to download images!
        save_input_representations() # generates features file for ManyNames
        save_input_representations_someRE() # generates features file for the ManyNames subset from from Mädebach et al., 2022 
    else: 
