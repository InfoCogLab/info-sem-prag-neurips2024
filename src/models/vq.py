...
class VQLayer(nn.Module):
    def __init__(self, num_protos, latent_dim, init_vectors=None, beta=0.25):
        ...
        if init_vectors is not None:
            ...
            self.prototypes.data = torch.randn(num_protos, latent_dim)
            self.prototypes.data /= self.prototypes.data.norm(p=2, dim=1, keepdim=True)
    ...
    def forward(self, latents, mus=None, logvar=None):
        ...
        vq_loss = 1.0 * (commitment_loss + 1.0 * embedding_loss)
