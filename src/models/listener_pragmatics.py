...
class ListenerPragmatics(nn.Module):
    ...
    def forward(self, reconstructions, features):
        # get target reconstruction only
        target_recons = reconstructions
...
class ListenerPragmaticsCosines(nn.Module):
    ...
    def forward(self, reconstructions, features):
        ...
        embedded_rec = self.comm_embedder(target_recons)
        num_imgs = features.shape[1]
        embedded_rec = embedded_rec.repeat(1, num_imgs, 1)
        embedded_features = self.feature_embedder(features)
        
        # Get cosine similarities
        cosines = self.cos(embedded_rec, embedded_features)
        logits = cosines
        return logits
